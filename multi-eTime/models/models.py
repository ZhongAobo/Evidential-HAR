...
class RandomDown(nn.Module):
    def __init__(self, kernel_size=2):
        super(RandomDown, self).__init__()
        self.kernel_size = kernel_size

    def forward(self, tensor):
        Bs, channel, length = tensor.shape
        assert length % self.kernel_size == 0, "Length must be even for this downsampling method."
        
        # 创建一个随机选择的索引张量
        indices = torch.randint(0, 2, (Bs, channel, length // self.kernel_size), device=tensor.device)
        
        # 生成相邻索引
        base_indices = torch.arange(0, length, self.kernel_size, device=tensor.device).unsqueeze(0).unsqueeze(0)
        base_indices = base_indices.expand(Bs, channel, -1)
        
        # 计算最终索引
        final_indices = base_indices + indices
        
        # 使用gather进行下采样
        downsampled_tensor = tensor.gather(2, final_indices)
...
class CNN(nn.Module):
    def __init__(self, configs):
        ...
        self.conv_block14 = nn.Sequential(
            nn.Conv1d(configs.input_channels, configs.mid_channels, kernel_size=configs.kernel_size,
                      stride=configs.stride, bias=False, padding=(configs.kernel_size // 2)),
            nn.BatchNorm1d(configs.mid_channels),
            nn.ReLU(),
        ...
        self.conv_block34 = nn.Sequential(
            nn.Conv1d(configs.mid_channels * 2, configs.final_out_channels, kernel_size=8, stride=1, bias=False,
                      padding=4),
            nn.BatchNorm1d(configs.final_out_channels),
            nn.ReLU(),
        ...
        self.fuse = nn.Linear(configs.final_out_channels * 3, configs.final_out_channels)
        ...
        if args.down == 'max':
            ...
            self.down1 = nn.AvgPool1d(kernel_size=2)
            self.down2 = nn.AvgPool1d(kernel_size=2)
        elif args.down == 'rand':
            self.down1 = RandomDown(kernel_size=2)
            self.down2 = RandomDown(kernel_size=2)
        elif args.down == 'max_cnn':
            self.down1 = nn.MaxPool1d(kernel_size=2)
            self.down2 = nn.Conv1d(configs.input_channels, configs.input_channels, kernel_size=8, stride=2, bias=False, padding=4)
        elif args.down == 'cnn':
            self.down1 = nn.Conv1d(configs.input_channels, configs.input_channels, kernel_size=8, stride=2, bias=False, padding=4)
            self.down2 = self.down1


    def forward(self, x_in):    #[B, C, 128]
        x_1 = x_in              #[B, C, 128]
        x_2 = self.down1(x_in)   #[B, C, 64]
        x_4 = self.down2(x_2)    #[B, C, 32]
       
        out = []

        x_4 = self.conv_block14(x_4)
        x_4 = self.conv_block24(x_4)
        x_4 = self.conv_block34(x_4)
        x_4 = self.adaptive_pool(x_4)
        x_4 = x_4.reshape(x_4.shape[0], -1)
        out.append(x_4)

        x_2 = self.conv_block12(x_2)
        x_2 = self.conv_block22(x_2)
        x_2 = self.conv_block32(x_2)
        x_2 = self.adaptive_pool(x_2)
        x_2 = x_2.reshape(x_2.shape[0], -1)
        out.append(x_2)

        x_1 = self.conv_block11(x_1)
        x_1 = self.conv_block21(x_1)
        x_1 = self.conv_block31(x_1)
        x_1 = self.adaptive_pool(x_1)
...
class classifier(nn.Module):
    def __init__(self, configs):
        ...
        self.logits0 = nn.Linear(configs.features_len * configs.final_out_channels, configs.num_classes)
        self.logits1 = nn.Linear(configs.features_len * configs.final_out_channels, configs.num_classes)
        self.logits2 = nn.Linear(configs.features_len * configs.final_out_channels, configs.num_classes)
        self.final_logits = nn.Linear(configs.features_len * configs.final_out_channels, configs.num_classes)
        self.configs = configs

    def forward(self, x, idx = -1):
        if idx == 0:
            predictions = self.logits0(x[idx])
        elif idx == 1:
            predictions = self.logits1(x[idx])
        elif idx == 2:
            predictions = self.logits2(x[idx])
        else:
            predictions = self.final_logits(x[idx])
        return predictions
...
class codats_classifier(nn.Module):
    def __init__(self, configs):
        ...
        self.logits2 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(self.hidden_dim, configs.num_classes))
        self.final_logits = nn.Sequential(
            nn.Linear(model_output_dim * configs.final_out_channels, self.hidden_dim),
            nn.ReLU(),
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.ReLU(),
...
class CNN_ATTN(nn.Module):
    ...
    def forward(self, x_in):
        ...
        x_4 = self.adaptive_pool(x_4)
        x_4 = x_4.reshape(x_4.shape[0], -1)
        x_4 = self.calculate_attentive_feat(x_4)
        out.append(x_4)
        ...
        x_2 = self.adaptive_pool(x_2)
        x_2 = x_2.reshape(x_2.shape[0], -1)
        x_2 = self.calculate_attentive_feat(x_2)
        out.append(x_2)
        ...
        x_1 = self.adaptive_pool(x_1)
        x_1 = x_1.reshape(x_1.shape[0], -1)
        x_1 = self.calculate_attentive_feat(x_1)
        out.append(x_1)

        x = torch.cat(out,dim=1)
        x = self.fuse(x)
        x = self.calculate_attentive_feat(x)
        out.append(x)
