...
class NO_ADAPT(Algorithm):
    ...
    def training_epoch(self,src_loader, trg_loader, avg_meter, epoch):
        for src_x, src_y in src_loader:
            ...
            evi_loss = 0.0
            if args.dist == 'nig':
                src_y_o = F.one_hot(src_y,num_classes=src_pred.shape[-1]).float()
                ...
                src_y_o = F.one_hot(src_y,num_classes=src_pred.shape[-1]).float()
                evi_loss += edl_digamma_loss(src_pred, src_y_o, epoch+1, src_pred.shape[-1], 10.0)
                ...
                src_y_o = F.one_hot(src_y,num_classes=src_pred.shape[-1]).float()
                evi_loss += edl_mse_loss(src_pred, src_y_o, epoch+1, src_pred.shape[-1], 10.0)

            elif args.dist == 'dir_log':
                src_y_o = F.one_hot(src_y,num_classes=src_pred.shape[-1]).float()
                evi_loss += edl_log_loss(src_pred, src_y_o, epoch+1, src_pred.shape[-1], 10.0)

            elif args.dist == 'fisher_edl':
                labels_1hot_ = F.one_hot(src_y,num_classes=src_pred.shape[-1]).float()
                evi_alp_ = F.softplus(src_pred) + 1
                evi_loss += fisher_edl_loss(evi_alp_, labels_1hot_,loss_type=args.loss_type)
...
class DANN(Algorithm):
    ...
    def training_epoch(self,src_loader, trg_loader, avg_meter, epoch):
        ...
        for step, ((src_x, src_y), (trg_x, _)) in joint_loader:
            ...
            # domain_loss = torch.zeros([1]).cuda().float() ###########################

            # loss = 1 * src_cls_loss + evi_loss * args.alpha

            # loss = args.a * src_cls_loss + evi_loss * args.alpha + \
            #     args.b * domain_loss
...
class CDAN(Algorithm):
    ...
    def training_epoch(self,src_loader, trg_loader, avg_meter, epoch):
        ...
        for step, ((src_x, src_y), (trg_x, _)) in joint_loader:
            ...
            #     args.c * loss_trg_cent + evi_loss * args.alpha
    ...
    def training_epoch(self, src_loader, trg_loader, avg_meter, epoch):
        ...
        for step, ((src_x, src_y), (trg_x, _)) in joint_loader:
            ...
            if args.dist == 'nig':
                ...
                labels_1hot_ = F.one_hot(src_y,num_classes=src_pred.shape[-1]).float()
                evi_alp_ = F.softplus(src_pred1) + 1
                evi_loss += fisher_edl_loss(evi_alp_, labels_1hot_,loss_type=args.loss_type)
                evi_alp_ = F.softplus(src_pred2) + 1
                evi_loss += fisher_edl_loss(evi_alp_, labels_1hot_,loss_type=args.loss_type)
...
def fisher_edl_loss(evi_alp_, labels_1hot_, loss_type='IEDL',target_c=1.0,fisher_c=0.1):
    evi_alp0_ = torch.sum(evi_alp_, dim=-1, keepdim=True)
    if loss_type=='EDL':
        loss_fisher_ = 0.0
        loss_mse_ = (labels_1hot_ - evi_alp_ / evi_alp0_).pow(2).sum(-1).mean()
        loss_var_ = (evi_alp_ * (evi_alp0_ - evi_alp_) / (evi_alp0_ * evi_alp0_ * (evi_alp0_ + 1))).sum(-1).mean()
    elif loss_type == 'IEDL':
    ...
    evi_loss = loss_mse_ + loss_var_ + fisher_c * loss_fisher_
    return evi_loss
